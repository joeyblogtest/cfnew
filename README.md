...
| `qj` | `no` | **可选**。降级控制，设置为`no`时启用降级模式：SOCKS5失败→fallback地址。 |
| `np` | `on` | **可选**。新协议控制，设置为`on`时启用新协议支持，默认关闭。 |
| `tp` | `on` | **可选**。Trojan协议控制，设置为`on`时启用Trojan over WebSocket支持，默认关闭。 |
| `tp_pass` | `你的密码` | **可选**。Trojan协议密码，56位SHA224哈希值。如果不设置，将使用UUID作为密码。 |
...
#### Trojan协议支持
- 支持Trojan over WebSocket协议
- 设置：`tp=on` 启用Trojan协议
- 密码：支持SHA224哈希密码或UUID
- 默认关闭，需要手动启用
- 与UUID完全兼容
